...
export class NoirGridVisualizer implements IVisualizer {
  ...
  render(context: RenderContext) {
    ...
    const bassEnergy = audio.bassEnergy ?? 0;
    const highEnergy = audio.highEnergy ?? 0;
    const gridSize = 50 + (bassEnergy * 20 * settings.intensity);
    ...
    for (let i = 0; i < cols; i++) {
      for (let j = 0; j < rows; j++) {
        ...
        // Izgara çizgileri
        ctx.strokeStyle = `rgba(228, 227, 224, ${0.1 + highEnergy * 0.2})`;
        ctx.strokeRect(x, y, gridSize, gridSize);

        // Sub-bass seviyesi yüksekse bazı kareleri rastgele doldur (Brütalist Glitch)
        if (bassEnergy > 0.7 && Math.random() > 0.8) {
