...
export class SimulationVisualizer implements IVisualizer {
  ...
  render(context: RenderContext) {
    ...
    const currentHeight = Math.max(1, eyeHeight * (1 - blinkFactor));
    ...
    // 2. Iris
    const irisRadius = Math.max(1, (eyeHeight * 0.8) / 2);
    ctx.beginPath();
    ...
    const pupilRadius = Math.max(1, irisRadius * 0.4 * this.pupilSize);
    ctx.beginPath();
    ...
    if (audio.beat || audio.hihat > 0.7) {
        ...
        for(let i=0; i<ringCount; i++) {
            ...
            ctx.beginPath();
            ctx.ellipse(centerX, centerY, Math.max(1, r), Math.max(1, r * 0.4), 0, 0, Math.PI * 2);
            ctx.stroke();
        }
    }
